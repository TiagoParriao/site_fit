...
export async function fetchGroupWaterHistory(supabase, members, preset, opts) {
  ...
  for (const row of data ?? []) {
    totals[row.user_id] ??= {}
    totals[row.user_id][row.data] = (totals[row.user_id][row.data] ?? 0) + row.ml
  ...
  const chartSeries = members.map((m) => {
    ...
    const totalMl = Object.values(dayTotals).reduce((sum, v) => sum + v, 0)
    return {
      user_id: m.user_id,
      nome: m.nome,
      cor: m.cor,
      avatarKey: m.avatar_key,
      values: chartDates.map((d) => Math.round((dayTotals[d] ?? 0) / 100) / 10),
      totalLitros: Math.round(totalMl / 100) / 10,
    }
  })
